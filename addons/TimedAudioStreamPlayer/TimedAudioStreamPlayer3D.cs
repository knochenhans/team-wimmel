...
public partial class TimedAudioStreamPlayer3D : AudioStreamPlayer3D
{
    ...
    public SoundSetResource CurrentSoundSet
    {
        ...
        set
        {
            ...
            if (currentSoundSet != null)
            {
                ...
                {
                    Logger.LogError("No TimedAudioStreamPlayerResource assigned to TimedAudioStreamPlayer3D.", Audio);
                    return;
                }
    ...
    public async void Play()
    {
        ...
        if (CurrentSoundSet == null)
        {
            ...
            if (CurrentSoundSet == null)
            {
                Logger.LogError("No sound set assigned to TimedAudioStreamPlayer3D.", Audio);
                return;
            }
        }
    ...
    public void SetCurrentSoundSetByID(string id)
    {
        ...
        if (soundSetDict.TryGetValue(id, out var soundSet))
            ...
            Logger.LogWarning($"No sound set with ID '{id}' found in TimedAudioStreamPlayer3D.", Audio);
    }
        ...
        foreach (var soundSet in SoundSets)
        {
            if (soundSet != null && !string.IsNullOrEmpty(soundSet.ID))
            {
                if (!soundSetDict.ContainsKey(soundSet.ID))
                    ...
                    Logger.LogWarning($"Duplicate sound set ID '{soundSet.ID}' found in TimedAudioStreamPlayer3D.", Audio);
            }
            else
            {
                Logger.LogWarning("Sound set with null or empty ID found in TimedAudioStreamPlayer3D.", Audio);
            }
        }
    }
